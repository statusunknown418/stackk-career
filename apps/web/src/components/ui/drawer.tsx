...
const DRAWER_POPUP_POSITION: Record<DrawerPosition, string> = {
	bottom:
	...
	top: "data-starting-style:transform-[translateY(calc(-100%-var(--inset)))] data-ending-style:transform-[translateY(calc(-100%-var(--inset)))] transform-[translateY(var(--drawer-swipe-movement-y))] border-b after:inset-x-0 after:bottom-full after:h-(--bleed) has-data-[slot=drawer-bar]:pb-2",
	left: "data-starting-style:transform-[translateX(calc(-100%-var(--inset)))] data-ending-style:transform-[translateX(calc(-100%-var(--inset)))] transform-[translateX(var(--drawer-swipe-movement-x))] w-[calc(100%-(--spacing(12)))] max-w-md border-e after:inset-y-0 after:end-full after:w-(--bleed) has-data-[slot=drawer-bar]:pe-2",
	right:
...
const DRAWER_POPUP_ROUNDED: Record<DrawerPosition, string> = {
...
const DRAWER_POPUP_BEFORE_ROUNDED: Record<DrawerPosition, string> = {
...
const DRAWER_POPUP_NESTED: Record<DrawerPosition, string> = {
	bottom:
	...
	top: "data-nested-drawer-open:transform-[translateY(calc(var(--drawer-swipe-movement-y)+var(--stack-peek-offset)+(var(--shrink)*var(--height))))_scale(var(--scale))] origin-[50%_var(--inset)]",
	left: "data-nested-drawer-open:transform-[translateX(calc(var(--drawer-swipe-movement-x)+var(--stack-peek-offset)))_scale(var(--scale))] origin-right",
	right:
...
function getDrawerPopupClasses(position: DrawerPosition, variant: "default" | "straight" | "inset"): string {
	const isVertical = position === "bottom" || position === "top";
	return cn(
		DRAWER_POPUP_BASE,
		"touch-none",
		DRAWER_POPUP_POSITION[position],
		variant !== "straight" && DRAWER_POPUP_ROUNDED[position],
		variant === "default" && DRAWER_POPUP_BEFORE_ROUNDED[position],
		variant === "inset" &&
		...
		<DrawerPortal {...portalProps}>
			...
			<DrawerViewport position={position} variant={variant}>
				<DrawerPrimitive.Popup
					className={cn(getDrawerPopupClasses(position, variant), className)}
