...
export interface HeroDitheringRootProps extends Omit<React.ComponentPropsWithoutRef<"section">, "title"> {
	...
	renderBadge?: (tech: HeroDitheringTechItem, index: number, defaultBadge: React.ReactNode) => React.ReactNode;
...
export interface HeroDitheringHeadingProps extends Omit<React.ComponentPropsWithoutRef<"div">, "title"> {
...
const defaultDescription = (
	<>
		Full-stack vercel ai sdk patterns for workflows, tool calling, and agent orchestration. Built with{" "}
...
const HeroDitheringContext = React.createContext<HeroDitheringContextValue | undefined>(undefined);

function useHeroDitheringContext() {
	const context = React.useContext(HeroDitheringContext);
	if (!context) {
		throw new Error("HeroDithering components must be used within HeroDitheringRoot");
	}
	return context;
}
...
export const HeroDitheringRoot = React.forwardRef<HTMLElement, HeroDitheringRootProps>(
	(
		{
			className,
			children,
...
export function HeroDitheringContainer({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
	return (
		<div
			className={cn(
...
export function HeroDitheringContent({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
	return (
		<div
			className={cn(
...
export function HeroDitheringDescription({
	...
	return (
		<div
			className={cn("mx-auto max-w-xl pb-2 text-center sm:pb-4 lg:mx-0 lg:max-w-none lg:pb-0 lg:text-left", className)}
			...
			{resolvedTechStack.map((tech, index) => {
				...
				const defaultBadge = (
					...
					>
						...
						{tech.version ? <span className="font-mono text-xs opacity-50">{tech.version}</span> : null}
					</Badge>
				);

				if (resolvedRenderBadge) {
					return <React.Fragment key={tech.name}>{resolvedRenderBadge(tech, index, defaultBadge)}</React.Fragment>;
				}
...
export function HeroDitheringMobileVisual({ className, mobileShaderProps, ...props }: HeroDitheringMobileVisualProps) {
	const context = useHeroDitheringContext();
		...
		<HeroDitheringRoot {...props}>
			<HeroDitheringContainer className={containerClassName}>
				<HeroDitheringContent className={contentClassName}>
					<HeroDitheringHeading className={headingWrapClassName} headingClassName={headingClassName} />
					<HeroDitheringDescription className={descriptionWrapClassName} descriptionClassName={descriptionClassName} />
...
export function NextjsIcon(props: SVGProps<SVGSVGElement>) {
	...
	return (
		<svg fill="none" height="1em" viewBox="0 0 180 180" width="1em" xmlns="http://www.w3.org/2000/svg" {...props}>
			<title>Next.js</title>
			<mask
				height={180}
				id={maskId}
			...
			<defs>
				<linearGradient gradientUnits="userSpaceOnUse" id={paint0Id} x1={109} x2={144.5} y1={116.5} y2={160.5}>
					<stop stopColor="white" />
					<stop offset={1} stopColor="white" stopOpacity={0} />
				</linearGradient>
				<linearGradient gradientUnits="userSpaceOnUse" id={paint1Id} x1={121} x2={120.799} y1={54} y2={106.875}>
					<stop stopColor="white" />
					<stop offset={1} stopColor="white" stopOpacity={0} />
				</linearGradient>
			</defs>
