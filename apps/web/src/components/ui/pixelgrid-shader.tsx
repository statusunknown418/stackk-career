...
function hexToRgb(hex: string): [number, number, number] {
	...
	// biome-ignore lint/suspicious/noBitwiseOperators: canonical hex-to-RGB extraction
...
function getBayer(ix: number, iy: number, matrix: Matrix): number {
	...
	if (matrix === "bayer2") {
		return B2[(iy % 2) * 2 + (ix % 2)] / 4;
	}
	if (matrix === "bayer4") {
		return B4[(iy % 4) * 4 + (ix % 4)] / 16;
	}
...
function fbm(x: number, y: number): number {
	return 0.5 * smoothNoise(x, y) + 0.35 * smoothNoise(2 * x, 2 * y) + 0.15 * smoothNoise(4 * x, 4 * y);
}
...
function evalShape(nx: number, ny: number, t: number, s: AppState): number {
	...
	if (shape === "wave") {
		const wave = Math.cos(frequency * 2.5 * nx * 4 - 2 * t) * Math.sin(frequency * 1.5 * nx * 4 + t) * amplitude;
		...
		const r = Math.sin((dist * 2) ** (2 + amplitude * 0.7) * rings * PI - frequency * t) * 0.5 + 0.2;
		v = r * smoothstep(0.5, 2, dist);
	} else if (shape === "swirl") {
		const l = Math.sqrt(nx * nx + ny * ny);
		const angle = rings * Math.atan2(ny, nx) + frequency * 1 * t;
		const twist = Math.max(amplitude * 1, 0.5);
		const offset = Math.max(l, 1e-6) ** -twist + angle / TWO_PI;
	...
	if (cursor.active) {
		...
		const ripple = Math.sin(scaledDist * 30 * s.cursorScale - t * 6 * s.cursorScale) * 0.5 + 0.5;
		const falloff = smoothstep(1.0, 0.0, scaledDist);
		if (cursor.mode === "ripple") {
			v = Math.min(1, v + ripple * falloff * 0.8);
		} else if (cursor.mode === "erase") {
			v *= 1 - falloff * 0.95;
		} else if (cursor.mode === "burn") {
			v = Math.min(1, v + falloff * 0.9);
		}
	}
...
export function PixelGridShader({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: updateCursor is stable (closure over refs only)
	const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
		const r = e.currentTarget.getBoundingClientRect();
		updateCursor((e.clientX - r.left) / r.width - 0.5, (e.clientY - r.top) / r.height - 0.5);
	}, []);

	return (
		<div className={className} ref={containerRef} style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
			<canvas
				onMouseLeave={() => {
