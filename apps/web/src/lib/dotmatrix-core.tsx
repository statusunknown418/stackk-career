...
const MATRIX_CELLS = FULL_INDEXES.map((index) => {
	const { row, col } = indexToCoord(index);
	return { col, index, key: `${row}-${col}`, row } as const;
});
...
function orderValueAt(order: readonly number[], index: number, label: string): number {
...
export function snakePathNormFromIndex(index: number): number {
	return orderValueAt(SNAKE_ORDER, index, "Snake order") / (CELLS - 1);
}

export function snakePathOrderValue(index: number): number {
	return orderValueAt(SNAKE_ORDER, index, "Snake order");
}
...
export function spiralInwardNormFromIndex(index: number): number {
	return orderValueAt(SPIRAL_INWARD_ORDER, index, "Spiral inward order") / (CELLS - 1);
}

export function spiralInwardOrderValue(index: number): number {
	return orderValueAt(SPIRAL_INWARD_ORDER, index, "Spiral inward order");
}
...
export function outerRingClockwiseOrderValue(index: number): number {
	return orderValueAt(OUTER_RING_CLOCKWISE_ORDER, index, "Outer ring clockwise order");
}
...
export function middleRingAntiClockwiseOrderValue(index: number): number {
	return orderValueAt(MIDDLE_RING_ANTI_CLOCKWISE_ORDER, index, "Middle ring anti-clockwise order");
}
...
export function diagonalSnakeOrderValue(index: number): number {
	return orderValueAt(DIAGONAL_SNAKE_ORDER, index, "Diagonal snake order");
}

export function diagonalSnakeNormFromIndex(index: number): number {
	return orderValueAt(DIAGONAL_SNAKE_ORDER, index, "Diagonal snake order") / (CELLS - 1);
}

function buildRowWaveSnakeOrderToIndexMap(): number[] {
	const order = new Array<number>(CELLS);
	const route: { col: number; dir: "up" | "down" }[] = [
		{ col: 0, dir: "up" },
		{ col: 2, dir: "down" },
		{ col: 1, dir: "up" },
		{ col: 3, dir: "down" },
...
export function rowWaveOrderValue(index: number): number {
	return orderValueAt(ROW_WAVE_SNAKE_ORDER, index, "Row wave snake order");
}
...
function getMatrix5Layout(size: number, dotSize: number, cellPadding?: number): { gap: number; matrixSpan: number } {
	const n = MATRIX_SIZE;
	if (cellPadding != null) {
...
export function DotMatrixBase({
	...
	const dots = MATRIX_CELLS.map(({ col, index, key, row }) => {
