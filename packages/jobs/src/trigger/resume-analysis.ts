...
import { RESUME_ANALYSIS_MODEL, RESUME_ANALYSIS_OBJECT_TYPE, runResumeAnalysisAgent } from "../agents/resume-analysis";
...
export const resumeAgentTask = schemaTask({
	...
	run: async ({ generationId, userId }, { ctx, signal }) => {
		...
		if (!file) {
			await markFailed({ generationId, userId, error: "Resume file not found" });
		...
		try {
			const result = runResumeAnalysisAgent({ pdfUrl: file.url, signal });
			...
			logger.info("[JOBS] resume-agent = completed", { generationId, usage });
			...
			logger.error("resume-agent = failed", { generationId, error: message });
			await markFailed({ generationId, userId, error: message });
			throw error;
		}
	},
});
