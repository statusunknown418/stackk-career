 ...
 * /letters/:generationId chat. The task runs streamText with the getUserMetadata
 * tool (the CV is injected directly into the user message) and emits the CoverLetter
 * via `Output.object`, streaming partial chunks to the frontend.
 *
...
export const caseyLettersInputSchema = z.object({
	...
	// Provenance of the job context (mirrors generations.jobContextSource). `resume-job-target`
	// → the summary is a normalized job posting; CASEY frames it as <TARGET_JOB> factual context
	// rather than untrusted <JOB_DESCRIPTION> user text.
	jobContextSource: z.enum(["manual", "resume-job-target"]).default("manual"),
